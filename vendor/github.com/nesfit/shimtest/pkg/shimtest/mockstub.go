...
func (stub *MockStub) PutPrivateData(collection string, key string, value []byte) error {
	ensureCollection(stub, collection)

	m, _ := stub.PvtState[collection]
	keys := stub.PvtKeys[collection]
...
func (stub *MockStub) GetPrivateDataByRange(collection, startKey, endKey string) (shim.StateQueryIteratorInterface, error) {
	...
	return NewMockPrivateDataRangeQueryIterator(stub, collection, startKey, endKey), nil
}
...
func (stub *MockStub) GetPrivateDataByPartialCompositeKey(collection, objectType string, attributes []string) (shim.StateQueryIteratorInterface, error) {
	...
	return NewMockPrivateDataRangeQueryIterator(stub, collection, partialCompositeKey, partialCompositeKey+string(utf8.MaxRune)), nil
}
...
func NewMockStub(name string, cc shim.Chaincode) *MockStub {
	...
	s.PvtKeys = make(map[string]*list.List)
...
/*****************************
 Private Data Range Query Iterator
*****************************/

// MockStateRangeQueryIterator ...
type MockPrivateDataRangeQueryIterator struct {
	Closed     bool
	Stub       *MockStub
	Collection string
	StartKey   string
...
// HasNext returns true if the range query iterator contains additional keys
// and values.
func (iter *MockPrivateDataRangeQueryIterator) HasNext() bool {
	if iter.Closed {
		// previously called Close()
		return false
	}
...
// Next returns the next key and value in the range query iterator.
func (iter *MockPrivateDataRangeQueryIterator) Next() (*queryresult.KV, error) {
	if iter.Closed == true {
		err := errors.New("MockPrivateDataRangeQueryIterator.Next() called after Close()")
		return nil, err
	}

	if iter.HasNext() == false {
		err := errors.New("MockPrivateDataRangeQueryIterator.Next() called when it does not HaveNext()")
		return nil, err
	}
	...
	err := errors.New("MockPrivateDataRangeQueryIterator.Next() went past end of range")
	return nil, err
}

// Close closes the range query iterator. This should be called when done
// reading from the iterator to free up resources.
func (iter *MockPrivateDataRangeQueryIterator) Close() error {
	if iter.Closed == true {
		err := errors.New("MockPrivateDataRangeQueryIterator.Close() called after Close()")
		return err
	}
...
// NewMockStateRangeQueryIterator ...
func NewMockPrivateDataRangeQueryIterator(stub *MockStub, collection string, startKey string, endKey string) *MockPrivateDataRangeQueryIterator {
	iter := new(MockPrivateDataRangeQueryIterator)
	iter.Closed = false
	iter.Stub = stub
	iter.Collection = collection
	iter.StartKey = startKey
	iter.EndKey = endKey
	iter.Current = stub.PvtKeys[iter.Collection].Front()
	return iter
}
...
func ensureCollection(stub *MockStub, collection string) {
	_, in := stub.PvtState[collection]
	if !in {
		stub.PvtState[collection] = make(map[string][]byte)
	}

	if stub.PvtKeys[collection] == nil {
		stub.PvtKeys[collection] = list.New()
	}
}
